...
1. **What would I do differently next time?**
   — The spec's Notes for the Implementer should flag that CLI ordering
   tests need the same §9 no-sleep SQL-backdate pattern as storage tests.
   The build discovered this mid-cycle; a one-line nudge ("if testing
   recency-ordered output, use raw SQL backdate — see §9 no-sleep rule")
   would eliminate the surprise.

2. **Does any template, constraint, or decision need updating?**
   — No new DEC. `project-state-note-shape` question closed at design
   (single free-text confirmed). The no-sleep-in-CLI-tests nudge is a
   WATCH candidate but not mid-stage codification.

3. **Is there a follow-up spec I should write now before I forget?**
   — None beyond the existing backlog. SPEC-031 (`brag project here`)
   is the natural next consumer of the project infrastructure.
