...
1. **What would I do differently next time?**
   — The `getCwd` indirection (`var getCwd = os.Getwd`) was not in the
   spec literal but required by the test harness. Future specs testing
   os-dependent calls (Getwd, Getenv) should call this out explicitly
   in Notes for the Implementer, alongside the cobra SilenceErrors +
   explicit Fprintln deviation pattern.

2. **Does any template, constraint, or decision need updating?**
   — DEC-019 is clean. The cobra SilenceErrors + explicit Fprintln
   pattern (deviation from bare UserErrorf) recurred here as in SPEC-030;
   it's a constraint-note candidate but not mid-stage codification.

3. **Is there a follow-up spec I should write now before I forget?**
   — SPEC-032 (`brag add --project` auto-fill) is the natural consumer —
   it calls `ProjectForPath` directly from its RunE. Already in backlog.
