...
- **Did we deliver the outcome in "What This Stage Is"?** **Yes, with one
  stated exception the stage itself overturned.** All four success criteria
  are met: `golangci-lint` gates CI with nine chosen linters at **0 issues**;
  coverage is measured at **83.5%** against an enforced **80.0%** floor;
  the coverage claim was mutation-checked in both directions; and `brag
  memory`'s header no longer uses the word its five siblings use for
  entries-in-scope. **The exception is the badge.** This section promised
  *"a badge"*; SPEC-082 Fork 3 decided **against** one and argued it from this
  repo's own derived-not-typed rule — a badge is a *cached* number served by a
  third party, and the first rule of the practices page is that a current
  number is derived. The stage's opening text was written before that argument
  existed. Recording it as an overturned premise rather than quietly dropping
  it, because "we said badge and shipped no badge" is exactly the kind of
  unexamined gap this stage exists to close.

- **How many specs did it actually take?** **Three against a plan of two** —
  SPEC-082 (lint + coverage), SPEC-084 (the `Entries:` envelope), and
  **SPEC-083, which the stage did not plan.** SPEC-083 exists because SPEC-082
  mechanised the `no-sql-in-cli-layer` constraint and deliberately refused to
  amend the rule it was mechanising, logging the scope question instead. That
  is the second consecutive stage to run three specs against a plan of two, and
  in both cases the third was **generated by the second refusing to widen its
  own scope** — SPEC-081 by SPEC-079's LD5, SPEC-083 by SPEC-082's LD7. That is
  the loop working, not estimation drift.

- **What changed between starting and shipping?** The stage opened believing
  its remaining defect was a capped number and closed having found it was a
  *pool-composition artifact that grows under filters* — the premise was wrong
  in the backlog entry, in the stage file, and in every handoff, and framing
  caught it by running the binary instead of reading the code.

- **Lessons that should update AGENTS.md, templates, or constraints?**
  - **The mutation protocol, promoted at N=2 — two halves of one rule.**
    *Confirm the mutant by content hash* (`git diff --quiet` is blind to
    untracked files, and a spec that creates one will report a real mutant as a
    missing one — SPEC-082), **and** *restore from a `/tmp` backup, never
    `git checkout`* (it discards uncommitted work; during a build most files
    carry some — SPEC-083). Landed in AGENTS.md §12 at this close.
  - **§9 half-(b) gains a third clause, promoted at N=4.** *Don't reason about
    which derived rows move — regenerate and diff.* SPEC-084 supplied both
    halves of the evidence at once: `812 → 815` was **fully derivable** (the
    spec specifies exactly three new test functions) and was still predicted
    wrong, while `778 → 781` cached a **derived property** of the thing being
    changed and shares no token with it, so no grep could have found it. Landed
    in AGENTS.md §9 at this close.
  - **`archive-spec` idempotency — fixed, not just recorded** (#188). It bit
    twice, at SPEC-083 ship and SPEC-084 ship. Routing it as a candidate after
    the first hit did not prevent the second; the guard did.

- **Should any spec-level reflections be promoted to stage-level lessons?**
  - **Yes: `Y3` and `X3` pin the same numbers by different means, and one of
    them should derive rather than cache.** `Y3` has now cached a value the
    shipping spec moves for **four consecutive specs** (SPEC-081, 082, 083,
    084). It is no longer a lesson about greps — it is a duplicated guard, and
    the §9 rule keeps catching it late instead of the design phase catching it
    early. **Held, not promoted, deliberately:** the fix is a change to
    `scripts/test-docs.sh`'s assertion design, which is spec work, not a
    one-line AGENTS.md rule. Routed to the next spec that opens that file.
  - **Yes, restated at stage scope: a guard that is green is not evidence its
    claim is true.** `AA3` is NOT-contains only; `Y1` asserts a package comment
    *exists*, not what it says; `X3` diffs a whole table and so absorbed two
    mispredictions without complaint. Every one behaved correctly and none of
    them would have caught the thing it sat next to. This is STAGE-021's
    recorded limit — *the guards reach numbers, not claims* — confirmed a
    second time rather than resolved.

- **What can a user do now that they couldn't before, at STAGE scope?**
  — Someone evaluating this repository can now **check its quality claims by
  running commands rather than by trusting prose**. Opening a pull request
  produces two machine verdicts that did not exist when the stage opened —
  nine chosen linters at **0 issues** and statement coverage against an
  enforced **80.0%** floor (83.5% today) — and a blocking architectural rule
  that had never been checked for the package its own path glob names now
  fails CI on either offending import, mutation-checked both ways. The stage
  also closed the last known correctness defect: `brag memory` reported
  `Entries: 200` against a 387-entry corpus while *growing* under filters, and
  now reports `Candidates:` with a decision record binding all six exporters
  forward. Evidence ref: `pr:188`.
