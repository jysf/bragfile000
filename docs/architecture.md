...
| `internal/memory` | (STAGE-019) Pure blended-ranking + token-budget package for `brag memory` — `Slice([]Entry, Options) Result` (reciprocal-rank fusion over recency/relevance/project, DEC-043), `RenderLine`, `EstimateTokens` (DEC-044). No SQL, no clock, no dependency outside `internal/storage` (for the `Entry` type). |
