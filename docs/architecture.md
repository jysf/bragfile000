...
| `internal/memory` | (STAGE-019) Pure blended-ranking + token-budget package for `brag memory` — `Slice([]Entry, Options) Result` (reciprocal-rank fusion over recency/relevance/project, DEC-043), `RenderLine`, `EstimateTokens` (DEC-044). `pool.go`'s `Gather` (SPEC-074 LD9) is the package's one I/O exception: the shared three-read pool composition, over a `Source` interface, that `brag memory` and the MCP resources/`brag_memory` all call so the "one composition, one tokenizer" parity claim holds. `Slice` itself stays pure; no clock, no dependency outside `internal/storage` (for the `Entry` type) and `internal/ftsquery`. |
| `internal/ftsquery` | (SPEC-072/SPEC-074, DEC-010) The extracted DEC-010 search-query transform — `Build(raw string) (string, error)`. Stdlib-only, pure leaf package (imports nothing internal); the third consumer (`internal/cli/search.go`, `internal/memory/pool.go`, `internal/mcpserver/server.go`) is why it moved out of `internal/cli` (DEC-024 revisit trigger (c)). |
| `internal/mcpserver` | (STAGE-009 / DEC-024; STAGE-019 / DEC-045) The `brag mcp serve` local stdio MCP server. Advertises five typed tools — `brag_add` / `brag_list` / `brag_memory` / `brag_search` / `brag_stats` — as thin wrappers over the same `Store`; SQL stays in `internal/storage`. `brag_add` stamps reserved `agent:<name>` / `model:<id>` provenance tags (DEC-024) so agent-authored entries are attributable with no schema change; `brag list --author agent\|human` reads that provenance back out. Also advertises the push surface: three `brag://` resources (`memory/recent`, `memory/project/{name}`, `projects`) an MCP client can auto-load with no tool call — the resource body is `export.ToMemoryMarkdown`'s output unmodified, so CLI↔MCP parity is by construction (DEC-045). The stdio transport owns stdout for protocol frames — the `stdout-is-for-data` spine generalized to a new transport. This is the "another frontend over `Store`" that Key Design Principle 2 anticipated. Bundled — with a `/brag` slash-command and a session-end capture-nudge hook — as an installable Claude Code plugin under `plugin/` (DEC-025). |
