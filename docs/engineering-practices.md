...
| Specs carried to ship and archived | 88 | `projects/*/specs/done/` |
| …of those, also carrying a build-phase reflection | 82 | `### Build-phase reflection` in those files |
...
overwriting it. This is a list of **named records, not a count**: the
inventory's `## Amendment` row counts a heading most of these records do not
use, so there is no counting rule to quote, and a keyword search would match
